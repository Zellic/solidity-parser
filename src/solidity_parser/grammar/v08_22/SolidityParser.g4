 ...
 */
constructorDefinition
locals[payableSet = False, visibilitySet = False]
:
	Constructor LParen (arguments=parameterList)? RParen
	(
		modifierInvocation
        | {not $payableSet}? Payable {$payableSet = True;}
		| {not $visibilitySet}? Internal {$visibilitySet = True;}
		| {not $visibilitySet}? Public {$visibilitySet = True;}
	)*
	body=block;
...
locals[
	...
	mutabilitySet = False,
	virtualSet = False,
	overrideSpecifierSet = False
]
:
	Function (identifier | Fallback | Receive)
	LParen (arguments=parameterList)? RParen
	(
		{not $visibilitySet}? visibility {$visibilitySet = True;}
		| {not $mutabilitySet}? stateMutability {$mutabilitySet = True;}
		| modifierInvocation
...
:
	...
	(
		{not $visibilitySet}? External {$visibilitySet = True;}
		| {not $mutabilitySet}? stateMutability {$mutabilitySet = True;}
		| modifierInvocation
		| {not $virtualSet}? Virtual {$virtualSet = True;}
		| {not $overrideSpecifierSet}? overrideSpecifier {$overrideSpecifierSet = True;}
	)*
	({$hasParameters}? Returns LParen returnParameters=parameterList RParen | {not $hasParameters}? )
	(Semicolon | body=block);
...
:
	...
	(
    	{not $visibilitySet}? External {$visibilitySet = True;}
		| {not $mutabilitySet}? Payable {$mutabilitySet = True;}
		| modifierInvocation
		| {not $virtualSet}? Virtual {$virtualSet = True;}
		| {not $overrideSpecifierSet}? overrideSpecifier {$overrideSpecifierSet = True;}
	 )*
 ...
 */
stateVariableDeclaration
locals [constantnessSet = False, visibilitySet = False, overrideSpecifierSet = False]
:
	type=typeName
	(
		{not $visibilitySet}? Public {$visibilitySet = True;}
		| {not $visibilitySet}? Private {$visibilitySet = True;}
		| {not $visibilitySet}? Internal {$visibilitySet = True;}
		| {not $constantnessSet}? Constant {$constantnessSet = True;}
		| {not $overrideSpecifierSet}? overrideSpecifier {$overrideSpecifierSet = True;}
		| {not $constantnessSet}? Immutable {$constantnessSet = True;}
	)*
	name=identifier
...
// SOLP only - Bibl
usingDirectiveAlias: identifierPath (As userDefinableOperator)? ;

/**
 * Using directive to attach library functions and free functions to types.
 * Can occur within contracts and libraries and at the file level.
 */
usingDirective: Using (identifierPath | (LBrace usingDirectiveAlias (Comma usingDirectiveAlias)* RBrace)) For (Mul | typeName) Global? Semicolon;
/**
...
elementaryTypeName[allowAddressPayable]: Address | {$allowAddressPayable}? Address Payable | Bool | String | Bytes | SignedIntegerType | UnsignedIntegerType | FixedBytes | Fixed | Ufixed;
functionTypeName
locals [visibilitySet = False, mutabilitySet = False]
:
...
mappingType: Mapping LParen key=mappingKeyType name1=identifier? DoubleArrow value=typeName name2=identifier? RParen;
/**
