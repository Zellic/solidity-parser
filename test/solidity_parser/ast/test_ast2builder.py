...
class TestSolidityTypeHelper(SnapshotTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.maxDiff = None
        self.vfs = VirtualFileSystem('../testcases/type_tests')
    ...
    def test_get_expr_type(self):
        ...
        cs = [c for u in ast1_nodes if u and not isinstance(u, solnodes1.PragmaDirective) for c in u.get_all_children() if isinstance(c, solnodes1.Stmt)]

        exprs = [e for c in cs for e in c.get_all_children() if isinstance(e, solnodes1.Expr) and not isinstance(e, solnodes1.Ident)]

        # test get_expr_type
        types = [(e, self.type_helper.get_expr_type(e, allow_multiple=True)) for e in exprs]

        self.assertMatchSnapshot(types)
